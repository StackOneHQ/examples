...
**stackone_execute** – Use when you have chosen a provider tool (e.g. from stackone_search) and need to run it. Pass the EXACT full tool name from stackone_search results (e.g. "googledocs_update_document") and the parameters it expects. Use document ids (e.g. remote_document_id from RAG or Available Documents) and construct the full request body matching the tool's schema. Use content from RAG results to populate parameters (e.g. text to find/replace). Do not ask the user for information you can infer from prior tool results.
