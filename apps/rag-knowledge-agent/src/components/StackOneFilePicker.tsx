...
export function StackOneFilePicker({ 
  ...
  useEffect(() => {
    ...
    const loadIntegration = async () => {
      try {
        ...
        const { integrations: list } = await res.json()
        const found = (list ?? []).find((i: { id: string }) => i.id === integrationId)
  ...
  useEffect(() => {
    ...
    const initializeAndOpenPicker = async () => {
      try {
        ...
        // Get or create a valid session token using the session manager (auth is enforced by API)
  ...
  }, [integration, isOpen, multiple, onClose, stackoneOnly])
