...
import { getSessionTokenForUser } from '@/utils/sessionUtils'
...
export function StackOneHub({ 
  ...
  const handleMessage = useCallback((event: MessageEvent) => {
    ...
    try {
      ...
      logger.error('StackOneHub: Error handling message', error)
    }
  ...
  const startFlow = useCallback(async () => {
    try {
      ...
      logger.error('StackOneHub: Error starting flow', error)
      if (onError) {
        onError(error)
      }
    }
