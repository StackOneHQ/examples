...
export default function NewAgentPage() {
  ...
  const loadData = async () => {
    try {
      const [integrationsRes, metadataRes] = await Promise.all([
        fetch('/api/integrations', { credentials: 'include' }),
        fetch('/api/stackone/connectors', { credentials: 'include' }),
      ])
      const integrationsData = integrationsRes.ok ? await integrationsRes.json() : { integrations: [] }
      const metadataData = metadataRes.ok ? await metadataRes.json() : []
      const integrationsList = (integrationsData.integrations ?? []).filter((i: { status: string }) => i.status === 'active')
      setIntegrations(integrationsList)
      setConnectorMetadata(Array.isArray(metadataData) ? metadataData : (metadataData as { connectors?: unknown[] })?.connectors ?? [])
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const integrationsPayload = Object.entries(agentIntegrations).map(([integrationId, fileIds]) => ({
        integration_id: integrationId,
        file_ids: fileIds,
      }))
      const putRes = await fetch(`/api/agents/${agent.id}/integrations`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!putRes.ok) {
        ...
        throw new Error(err?.error ?? 'Failed to set up integrations')
      }
