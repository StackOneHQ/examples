...
export default function Dashboard() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const [agentsRes, integrationsRes, threadsRes] = await Promise.all([
        fetch('/api/agents', { credentials: 'include' }),
        fetch('/api/integrations', { credentials: 'include' }),
        fetch('/api/threads/recent', { credentials: 'include' }),
      ])
      ...
      if (threadsRes.ok) {
        const { threads: threadsData } = await threadsRes.json()
