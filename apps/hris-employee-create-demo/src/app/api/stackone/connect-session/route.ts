...
export async function POST(request: NextRequest) {
  try {
    ...
    const metadata = { 
      source: 'hris-employee-create-demo',
      user_id: origin_owner_id 
    }

    // Prepare payload for StackOne API
    const payload = {
    ...
    try {
      ...
      console.error('Error calling StackOne API:', e)
      return NextResponse.json({ error: 'Error when trying to fetch session' }, { status: 500 })
    }

  } catch (error) {
    console.error('Error in connect session API:', error)
