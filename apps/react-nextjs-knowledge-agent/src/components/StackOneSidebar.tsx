...
export default function StackOneSidebar({ user, collapsed: externalCollapsed, onCollapse }: StackOneSidebarProps) {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Clear all StackOne sessions before logout
      sessionManager.clearAllSessions()
      const { error } = await supabase.auth.signOut()
      if (error) {
        console.error('Logout error:', error)
      } else {
        console.log('Logout successful, auth state change should trigger redirect')
      }
      // The auth state change handler in ClientLayout will handle the redirect
    } catch (error) {
      console.error('Logout error:', error)
    }
  }
